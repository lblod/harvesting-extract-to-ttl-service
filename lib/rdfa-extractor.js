...
import { WRITE_DECISION_FILES } from "../constants";
...
class SourceAwareStoreWriter extends Writable {
  ...
  async _write(data, _enc, next) {
    if (
      ...
      // Skip adding decision content entirely when file writing is disabled
    } else {
...
export default class RDFAextractor {
  async extractPage(task, filePath, fileMetadata) {
    ...
    try {
      ...
      let window = new JSDOM(html?.replace(new RegExp("\uFFFD", "g"), ""))
        .window;
