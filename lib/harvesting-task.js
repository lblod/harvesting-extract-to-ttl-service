...
async function insertInvalidTriples(triples) {
  ...
  for(let i = 0; i < triples.length ; i++){
    try {
      ...
      const unparsedPred = getUnparsedPred(parsedTriple.predicate);
      const tripleString = getStringRepresentation(parsedTriple.subject, N3.DataFactory.namedNode(unparsedPred), N3.DataFactory.literal(parsedTriple.object.value))
...
async function getUnparsedPred(predicate) {
  const queryResult = await query(`
    SELECT ?predicate FROM {
      ?predicate <http://data.lblod.info/ns/predicates/unparsedFormOf> ${predicate.value}.
    }
  `)
  if(queryResult.bindings[0]) {
    return queryResult.bindings[0].predicate.value
  } else {
    const unparsedPred = `http://data.lblod.info/ns/predicates/${uuid()}`
    const labelQueryResult = await query(`
      SELECT ?label FROM {
        ${predicate.value} <http://www.w3.org/1999/02/22-rdf-syntax-ns#label> ?label
      }
    `)
    const label = labelQueryResult.bindings[0].label.value;
    await query(`
      INSERT DATA {
        GRAPH ${sparqlEscapeUri(TARGET_GRAPH)} {
          ${unparsedPred} <http://data.lblod.info/ns/predicates/unparsedFormOf> ${predicate.value};
