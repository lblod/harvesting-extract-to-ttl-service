...
import { getFileContent, getFileMetadata } from "./file-helpers";
...
export async function importHarvestingTask(taskURI) {
  ...
  let graph = await getGraph(taskURI);
  let extractor = await constructExtractor(pages);
  await importTTL(extractor, graph);
}
...
async function correctAndRepairTriples(ttlTriples, graph) {
  const validTriples = [];
...
async function importTriples(graph, ttlTriples){
  const triples = [...ttlTriples]; //duplicate so we splice
  while(triples.length) {
    const batch = triples.splice(0, 100);
    const queryStr = `
      INSERT DATA {
...
//         `);
//       }
//       await insertInvalidTriples(invalidTriples, graph);
//     }
//     catch(e) {
//     console.log(e);
//   }
...
//  */
// async function insertInvalidTriples(triples, graph) {
