...
export async function importHarvestingTask(taskURI) {
  try {
    ...
    await updateTaskStatus(taskURI, TASK_SUCCESS);
  } catch (e) {
    console.log(`Something went wrong while importing the harvesting task <${taskURI}>`);
    console.error(e);
    try {
      await updateTaskStatus(taskURI, TASK_FAILURE);
    } catch (e) {
