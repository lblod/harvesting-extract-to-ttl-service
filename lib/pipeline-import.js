...
import { extractBasenameFromPath, getFilePath, getFileMetadata, writeTtlFile } from './file-helpers';
...
export async function run(deltaEntry) {
  ...
  try {
    ...
    for (const page of pages) {
      ...
      const originalFile = await writeTtlFile(task.graph, ttl, `${basename}-original.ttl`, page);
      await appendTaskResultFile(task, fileContainer, originalFile, page);
      const { validTriples, invalidTriples, correctedTriples } = await correctAndRepairTriples((ttl || []));
      const validFile = await writeTtlFile(task.graph, validTriples, `${basename}-valid.ttl`, page);
      await appendTaskResultFile(task, fileContainer, validFile);
      const invalidFile = await writeTtlFile(task.graph, invalidTriples, `${basename}-invalid.ttl`, page);
      await appendTaskResultFile(task, fileContainer, invalidFile);
      const correctedFile = await writeTtlFile(task.graph, correctedTriples, `${basename}-corrected.ttl`, page);
