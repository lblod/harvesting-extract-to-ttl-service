...
import { getFilePath, getFileMetadata, writeFileToTriplestore, appendTempFile } from './file-helpers';
...
export async function run(deltaEntry) {
  ...
  try {
    ...
    for (const page of pages) {
      ...
      const store = await extractor.extractPage(task, path, metaData);
    ...
    const fileUri = await writeFileToTriplestore(task.graph, orginalTempFilePath, 'original.ttl');
    ...
    const validFile = await writeFileToTriplestore(task.graph, validTempFilePath, 'valid-triples.ttl');
    await appendTaskResultFile(task, fileContainer, validFile);

    const inValidFile = await writeFileToTriplestore(task.graph, invalidTempFilePath, 'invalid-triples.ttl');
    await appendTaskResultFile(task, fileContainer, inValidFile);

    const correctedFile = await writeFileToTriplestore(task.graph, correctedTempFilePath, 'corrected-triples-[original].ttl');
