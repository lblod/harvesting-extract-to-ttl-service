...
export async function run(deltaEntry) {
  ...
  try {
    ...
    const countTriples = await countPages(task);
    ...
    for (let i = 0; i <= pagesCount; i++) {
      const offset = i * defaultLimitSize;
      const pages = await getPages(task, defaultLimitSize, offset);
      for (const page of pages) {
        ...
        const originalFile = await writeTtlFile(
          task.graph,
          ttl,
 ...
 */
async function getPages(task, limit, offset) {
  const result = await query(`
  ${PREFIXES}
