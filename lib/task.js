...
export async function deleteResultContainer(task) {
    const queryStr = `
    PREFIX dct: <http://purl.org/dc/terms/>
    PREFIX task: <http://redpencil.data.gift/vocabularies/tasks/>
    ...
    DELETE WHERE {
      GRAPH ${sparqlEscapeUri(task.graph)} {
        ${sparqlEscapeUri(task.uri)} task:resultsContainer ?container.
        ?container a nfo:DataContainer; ?cp ?co.
...
export async function loadExtractionTask( subject ){
  const queryTask = `
   ${PREFIXES}
   SELECT DISTINCT ?graph ?task ?id ?job ?created ?modified ?status ?index ?error WHERE {
    GRAPH ?graph {
