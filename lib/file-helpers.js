...
import {GZIP_OUTPUT_FILES} from "../constants";
...
export async function writeTtlFile(graph, content, logicalFileName) {
  ...
  const now = new Date();
  console.log(`ZIP? ${JSON.stringify(GZIP_OUTPUT_FILES)} Files are ${phyFilename} (phy) and ${logicalFileName} (log)`)

  try {
    if (GZIP_OUTPUT_FILES) {
      const contentStream = Readable.from(content);
      const destination = createWriteStream(path);
      console.log(`GZIP: output to file ${path}`)
      await contentStream.pipe(zlib.createGzip({level: 9})).pipe(destination);
    }
